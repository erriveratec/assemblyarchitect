...
/* Function: check_is_register
 * -----------------------------------------------------------------------------
 * This function verifies is an operand id is a register
 *
 * Arguments:
 *  id: The operand id that will be verified.
 *	
 ...
 * -----------------------------------------------------------------------------
 * This function verifies if when changing or assigning an operando, it e
 *
 * Arguments:
...
bool check_operand_compatilibity(operand_t *op, code_line_t *line)
{
	...
	if (check_is_register(op->id) == true){
		return true;
	}
	...
	switch(line->state){
		case MISSING_BOTH:
			...
			break;
		case MISSING_OP1:
			if (check_is_register(line->op2->id) == true && op->id != IB){
				compatible = true;
			}
			break;
		case MISSING_OP2:
			if (check_is_register(line->op1->id) == true && op->id != OB){
				compatible = true;
			}			
			break;
		case CHANGING_OP1:
