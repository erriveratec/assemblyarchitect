...
static ir_rules_t g_base_rules = {.instruction_limit       = 0,
                                  .code_editable           = true,
...
static ir_rules_t g_current_rules = {.instruction_limit       = 0,
                                     .code_editable           = true,
                                     .code_editable_exception = IR_NO_EXCEPTION,
                                     .buffer_selectable       = true,
                                     .register_selectable     = true,
...
static bool ir_rules_are_valid(const ir_rules_t *rules)
{
	if (rules == NULL) {
		return false;
	}
...
const ir_rules_t *ir_get_base_rules(void) { return &g_base_rules; }

const ir_rules_t *ir_get_current_rules(void) { return &g_current_rules; }
...
int ir_get_instruction_limit(void) { return g_current_rules.instruction_limit; }

bool ir_is_over_instruction_limit(int instruction_count)
{
	return instruction_count > g_current_rules.instruction_limit;
}
...
bool ir_is_code_editable(void)
{
	...
	if (exception == IR_INSTRUCTION_EXCEPTION && iw_chk_click_ins()) {
		return true;
	}
	...
	if (exception == IR_OPERAND_2_LAST) {
		...
		return cw_chk_click_code_op2(code_size);
	}
	...
	if (cw_chk_click_code()) {
		...
		int position = cw_get_code_line_pos_by_ptr(line);
...
bool ir_is_delete_enabled(void) { return g_current_rules.delete_enabled; }
...
bool ir_is_arrange_enabled(void) { return g_current_rules.arrange_enabled; }
...
bool ir_is_buffer_selectable(void) { return g_current_rules.buffer_selectable; }