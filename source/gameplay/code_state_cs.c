...
void lv_set_hold_line(code_line_t *line) { g_hold_line = line; }

code_line_t *get_hold_line(void) { return g_hold_line; }

bool chk_player_holds_line(void) { return g_hold_line != NULL; }
...
cs_context_t cs_capture_context(void)
{
	...
	if (context.code_size > 0) {
		code_line_t *last_line = cw_get_code_line_at_pos(context.code_size - 1);
